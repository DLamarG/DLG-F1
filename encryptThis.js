var encryptThis = function(text) {
   ...
   for (let i=0;i<words.length;i++){
     current=words[i].toString(),first=current[0].charCodeAt()
     sec=current[1],last=current[current.length-1]
     middle=current.substring(2,current.length-1)
     neWrd=first+last+middle+sec
     if(current.length>=3){
       rRR.push(neWrd)
     }
     if(current.length==2){
       rRR.push(first.toString()+sec)
     }
     if(current.length==1){
       rRR.push(first.toString())
     }
   }
   return rRR.join(' ')
 }
